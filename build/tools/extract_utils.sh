...
function setup_vendor() {
    ...
    export PHENOM_ROOT="$3"
    if [ ! -d "$PHENOM_ROOT" ]; then
        echo "\$PHENOM_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$PHENOM_ROOT/$OUTDIR" ]; then
        mkdir -p "$PHENOM_ROOT/$OUTDIR"
    fi

    export PRODUCTMK="$PHENOM_ROOT"/"$OUTDIR"/"$DEVICE"-vendor.mk
    export ANDROIDMK="$PHENOM_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$PHENOM_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$PHENOM_ROOT"/"$OUTDIR"/proprietary
    if [ "$SRC" = "adb" ]; then
        init_adb_connection
    fi
